...
contract VotingSystem {
    ...
    mapping(string => bool) private voterNationalIDExists; // Prevent duplicate voter National IDs
    ...
    function vote(address _candidate) external duringVoting {
        ...
        string memory voterArea = sender.area;
        require(
            keccak256(abi.encodePacked(voterArea)) == keccak256(abi.encodePacked(candidates[_candidate].area)),
            "Candidate and voter must be in the same area."
        );

        // Ensure the votes in this area do not exceed maxVoters
        Area storage area = areas[voterArea];
        ...
        emit VoteCasted(msg.sender, _candidate, voterArea);
    }
